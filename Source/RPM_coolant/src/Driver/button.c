...
static void ScanTimerCallback(void)
{
    ...
    for (fu8ButtonIndex = 0; fu8ButtonIndex < BUTTON_COUNT; fu8ButtonIndex++)
    {
        ...
        else
        {
            ...
            pstcButton->u16countLongPres = 0;
            pstcButton->u8interval = 0;
        }
        ...
        if ((enSampledState != pstcButton->enCurrState) || (pstcButton->u16stateRepeat == 50)) 
        {
            ...
            if (pstcButton->enCurrState == StateLong)
            {
            	...
            	//Если кнопка нажата больше 3 раз
            	else
            	{
            		if (pstcButton->u8interval < (250/BUTTON_SCAN_INTERVAL_MS))
            		{
            			pstcButton->u8interval++;
            			return;            			
            		}
            ...
            {
                // Accept new state
                //Обработка отпускания кнопки при долгом нажатии
                if ((pstcButton->enCurrState == StateLong) && (enSampledState != pstcButton->enCurrState))
                	pstcButton->enCurrState = enSampledState;
                else
                {
                	pstcButton->enCurrState = enSampledState;
